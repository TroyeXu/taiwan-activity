...
const taiwanCities = {
  north: [
    ...
    {
      city: '桃園市',
      districts: ['桃園區', '中壢區', '平鎮區', '楊梅區', '龜山區', '八德區', '大溪區', '龍潭區'],
    },
    { city: '新竹市', districts: ['東區', '北區', '香山區'] },
    {
      city: '新竹縣',
      districts: ['竹北市', '湖口鄉', '新豐鄉', '新埔鎮', '關西鎮', '芎林鄉', '寶山鄉'],
    },
  ],
  central: [
    {
      city: '苗栗縣',
      districts: ['苗栗市', '頭份市', '竹南鎮', '後龍鎮', '通霄鎮', '苑裡鎮', '銅鑼鄉'],
    },
    {
      city: '台中市',
      districts: [
    ...
    {
      city: '彰化縣',
      districts: ['彰化市', '員林市', '鹿港鎮', '和美鎮', '溪湖鎮', '二林鎮', '田中鎮'],
    },
    {
      city: '南投縣',
      districts: ['南投市', '埔里鎮', '草屯鎮', '竹山鎮', '集集鎮', '名間鄉', '鹿谷鄉', '中寮鄉'],
    },
    {
      city: '雲林縣',
      districts: ['斗六市', '斗南鎮', '虎尾鎮', '西螺鎮', '土庫鎮', '北港鎮', '麥寮鄉'],
    },
  ],
  south: [
    ...
    {
      city: '嘉義縣',
      districts: ['太保市', '朴子市', '布袋鎮', '大林鎮', '民雄鄉', '新港鄉', '水上鄉'],
    },
    {
      city: '台南市',
      districts: [
    ...
    {
      city: '屏東縣',
      districts: ['屏東市', '潮州鎮', '東港鎮', '恆春鎮', '萬丹鄉', '長治鄉', '內埔鄉', '里港鄉'],
    },
  ],
  east: [
    {
...
function generateActivityTime(baseDate: Date) {
  ...
  return {
    ...
    endTime:
      startHour && duration_hours
        ? `${(startHour + duration_hours).toString().padStart(2, '0')}:00`
        : null,
  ...
  try {
    ...
    for (const [region, cities] of Object.entries(taiwanCities)) {
      for (const cityData of cities) {
        const activitiesPerCity = Math.ceil(
          targetCount / Object.values(taiwanCities).flat().length
        );
        ...
        for (let i = 0; i < activitiesPerCity && activityCount < targetCount; i++) {
          ...
          const district =
            cityData.districts[Math.floor(Math.random() * cityData.districts.length)];
          ...
          const description = descTemplate!
            ...
            .replace(
              '{category}',
              categoryData.find((c) => c.slug === template!.category)?.name || ''
            )
          ...
          // 20% 機率有第二個分類
          if (Math.random() < 0.2) {
            const otherCategories = Object.keys(categoryMap).filter(
